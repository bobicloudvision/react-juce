...
```bash
$ git clone --recurse-submodules git@github.com:bobicloudvision/react-juce.git
```

or

```bash
$ git clone git@github.com:bobicloudvision/react-juce.git
$ cd react-juce
$ git submodule update --init --recursive
```

Note that the `git@github.com` prefix here indicates cloning via SSH. If you prefer
to work with git via HTTPS you'll want to swap in `https://github.com/bobicloudvision/react-juce.git`
